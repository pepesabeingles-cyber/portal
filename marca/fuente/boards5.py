...
# 1 · logo
if want("logo"):
    b = f'''<div class=b style="background:{HUE};display:flex;align-items:center;justify-content:center">{inl(layers(WV,"color"), h=640)}
...
if want("idea"):
    s = 2.9; ox, oy = 170, 220
    owl = inl(layers(None, "color"), h=OH * s)
    ob = lk.bounds(SYM)
    px = lambda x: ox + (x - ob[0]) * s
    py = lambda y: oy + (y - ob[1]) * s
    ...
    lab = lambda x, y, t: f'<div class=lab style="position:absolute;left:{x}px;top:{y-8}px;width:320px;color:{TIN}">{t}</div>'
    xr = px(70) + 40
    pts = [((46.5, 40), "Mirada tranquila"), ((52, 59), "Disco facial · línea fina"), ((64, 100), "Alas plegadas"), ((56, 146.5), "Posado en la raya del libro")]
    ann = "".join(dot(px(x), py(y)) + L(px(x), py(y), xr) + lab(xr + 12, py(y), t) for (x, y), t in pts)
    txt = f'''<div style="position:absolute;left:960px;top:250px;width:560px;display:flex;flex-direction:column;gap:24px">
      <div class=serif style="font-size:50px;font-weight:500;line-height:1.08;color:{PET}">Un búho que acompaña.</div>
      <div style="font-size:20px;line-height:1.55">El nombre lleva la marca: Gabinete Contable en Bodoni, sobrio y con autoridad. El búho está a su lado, de pie y sereno: el saber que vigila las cuentas sin robarse la atención.</div>
      <div style="font-size:20px;line-height:1.55">Esbelto y de mirada tranquila. El disco facial y las alas se trazan con la misma línea fina de la letra, y se posa sobre una raya, como la que cierra una cuenta en el libro.</div>
      <div style="width:80px;height:2px;background:{ORO}"></div>
      <div style="font-size:16px;line-height:1.5;color:{NIE}">En negativo, el búho va en oro sobre petróleo.</div></div>'''
    bb = f'<div class=b style="background:{HUE}"><div style="position:absolute;left:{ox}px;top:{oy}px">{owl}</div>{ann}{txt}{tag("La idea")}{tag("02", x=1510)}</div>'
...
if want("versiones"):
    ...
    sizes = "".join(f'<div style="display:flex;flex-direction:column;align-items:center;gap:6px">{inl([(SMALL if s<=24 else SYM, PET)], h=s)}<span style="font-size:11px;color:{NIE}">{s} px</span></div>' for s in (16, 24, 40, 64))
    ...
    sizes += f'<div style="display:flex;flex-direction:column;align-items:center;gap:6px">{inl(layers(WH,"color"), h=32)}<span style="font-size:11px;color:{NIE}">horizontal a 32 px</span></div>'
    b = f'''<div class=b style="background:#EAE5DB;padding:96px 64px 0 64px;display:grid;grid-template-columns:460px 460px 460px;gap:26px 32px">
      {cell(inl(layers(WV,"color"), w=300), HUE, "Principal")}
      {cell(inl(layers(WH,"color"), w=360), HUE, "Horizontal · web y documentos")}
      {cell(inl(layers(WV,"negativo"), w=300), PET, "Negativo · búho en oro")}
      {cell(inl(layers(None,"oro"), h=190), PETD, "El búho · avatar y sello", w=460, h=250)}
...
if want("apps1"):
    ...
    card_front = f'<div style="width:504px;height:280px;background:{PET};box-shadow:0 22px 40px rgba(10,30,38,.35);display:flex;align-items:center;justify-content:center">{inl(layers(WV,"negativo"), h=200)}</div>'
...
if want("apps2"):
    ...
    letter = f'''<div style="width:400px;height:566px;background:#FBF9F5;box-shadow:0 18px 36px rgba(10,30,38,.22);padding:44px 46px;display:flex;flex-direction:column">
      <div style="display:flex;justify-content:center">{inl(layers(WV,"color"), h=120)}</div>
    ...
    web = f'''<div style="width:640px;height:400px;background:{HUE};box-shadow:0 18px 36px rgba(10,30,38,.22);overflow:hidden">
      ...
      <div style="padding:20px 30px;display:flex;justify-content:space-between;align-items:center">{inl(layers(WH,"color"), h=40)}
